...
def main():
    while True:
        ...
        while user is None or mode is None:
            ...
            single_btn = Button(width / 8, height / 2 + 80, width / 4, 50, "Single Player", mediumFont)
            multi_btn = Button(5 * width / 8, height / 2 + 80, width / 4, 50, "Multiplayer", mediumFont)

            for btn in [play_x_btn, play_o_btn, single_btn, multi_btn]:
                btn.check_hover(mouse)
                btn.draw(screen)

            # Cursor change on hover
            if any([play_x_btn.hovered, play_o_btn.hovered, single_btn.hovered, multi_btn.hovered]):
        ...
        while True:
            ...
            # Cursor change on in-game buttons
            hovered_buttons = [pause_button, resume_button, restart_button, easy_btn, medium_btn, hard_btn]
            if game_over:
                next_btn = Button(width / 2 - 150, height - 70, 150, 50, "Play Again" if rounds_played >= total_rounds - 1 else "Next Round", scoreFont, hover_color=green)
                quit_btn = Button(width / 2 + 30, height - 70, 120, 50, "Quit", scoreFont, hover_color=red)
                hovered_buttons += [next_btn, quit_btn]
            for btn in hovered_buttons:
                btn.check_hover(mouse)
            if any(btn.hovered for btn in hovered_buttons):
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_HAND)
            else:
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_ARROW)
